...
local elapsed = 0
local screenshotPath = ".tmp/pcgDungeon-light-debug.png"
...
function Start()
    local ok, err = xpcall(function()
        ...
        local light = pcgDungeonRenderer:AddPointLight(parent, {
        ...
        assert(#pcgDungeonRenderer.lights == 1, "point light was not registered for debug drawing")
        ...
        assert(pcgDungeonRenderer.lightDebugRoot ~= nil, "light debug geometry was not created")
        SubscribeToEvent("Update", "HandleLightDebugUpdate")
    end, debug.traceback)

    if not ok then
        ErrorExit("[pcgDungeon-light-debug] FAIL\n" .. tostring(err), 1)
        return
    end
end
...
function HandleLightDebugUpdate(eventType, eventData)
    ...
    screenshot:Dispose()
    if not saved then
        ErrorExit("[pcgDungeon-light-debug] FAIL screenshot capture", 1)
        return
    end
    print("[pcgDungeon-light-debug] PASS point position and radius debug geometry")
