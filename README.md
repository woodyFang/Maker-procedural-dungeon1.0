...
- 项目长度单位为米；`mesh_info` 中的 Unreal 坐标、尺寸和偏移使用厘米
...
```text
PCGDungeonGenerator
  -> PCGDungeonMarkerPipeline
  -> PCGDungeonMeshInfoAdapter
  -> PCGDungeonRenderer
```

调整颜色或点击“刷新地牢”会基于最新拓扑重新构建模型、门和 Marker 点光源。
内部的历史参考验证、方块调试、灯光调试和生成参数不在正式 UI 中显示，
底层接口及测试仍然保留。
...
文件：`wuzhong/assets/PCGDungeon/PCGDungeon.mesh_info.json`

该清单控制：

- Unreal 资源标识到 UrhoX Model/Material 的映射
...
```text
wuzhong/
  assets/PCGDungeon/   mesh_info、灯光清单与历史回归参考
  Materials/            UrhoX 材质
  Models/               古堡模型
  Textures/             纹理资源
  scripts/App/          应用流程
  scripts/Generation/   地牢、A*、Marker 和坐标转换
  scripts/Rendering/    原生与 PCG Dungeon 渲染器
...
- `test_pcg_dungeon_integration.lua`：拓扑、Marker、模型、灯光、刷新和调试状态
- `test_shadow_castle_lighting.lua`：环境光、Marker 点光和题材切换
- `test_pcg_dungeon_light_parity.lua`：灯光清单与 UrhoX Light 映射
...
- 引入暗影古堡运行时生成与 PCG Dungeon/Marker 渲染流程
